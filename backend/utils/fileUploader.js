...
const fileUploader = (req, res) => {
  try {
    ...
    upload(req, res, (err) => {
      ...
      const fileUrl = `${process.env.SERVER_URL}/public/health&ipd/${req.file.filename}`;
...
const uploadPatientRecords = (req, res) => {
  try {
    ...
    upload(req, res, async (err) => {
      ...
      const patientPhoneNumbers = patients.map(
...
function parseAgeSex(ageSex) {
  ...
  if (!match) return { age: null, sex: "Other" };

  const age = parseInt(match[1], 10);
  const sex =
    match[2].toUpperCase() === "M"
      ? "Male"
      : match[2].toUpperCase() === "F"
      ? "Female"
      : "Other";
