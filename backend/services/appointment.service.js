...
const bookAppointment = async (appointmentData, doctorId) => {
  try {
    const { name, phoneNumber } = appointmentData;
    ...
    if (!patient) {
      ...
      const doctorPatient = await DoctorPatient.findOne({ doctorId, patientId: patient._id });
...
const getAllUpcomingAppointments = async (doctorId, page = 1, limit = 10) => {
  try {
    if (!doctorId) {
      return {
        statusCode: 400,
...
const getAllPastAppointments = async (doctorId, page = 1, limit = 10) => {
  try {
    if (!doctorId) {
      return {
        statusCode: 400,
    ...
    let appointments = await Appointment.find({
      ...
      $or: [
        { date: { $lt: todayStart } },
        { date: todayStart, time: { $lt: currentTime } },
      ],
    })
      .sort({ date: -1, time: -1 })
      .populate('patientId')
...
const getAppointmentsByDateRange = async (doctorId, fromDate, toDate, page = 1, limit = 10) => {
  try {
    if (!doctorId || !fromDate || !toDate) {
      return {
        statusCode: 400,
        error: 'DoctorId, fromDate, and toDate are required',
      };
    }
    ...
    if (start > end) {
      return {
        statusCode: 400,
        error: 'Invalid date range. fromDate must be less than or equal to toDate.',
      };
    }
...
const getAppointmentTimeSlots = async (doctorId, locationId, date) => {
  try {
    ...
    const parseTime = (timeStr) => {
      ...
      else if (meridian === 'AM' && hours === 12) hours24 = 0;
      
      return { hours: hours24, minutes };
    };

    const fromTime = parseTime(location.from);
    const toTime = parseTime(location.to);
    ...
    startDate.setHours(fromTime.hours, fromTime.minutes, 0, 0);

    const endDate = new Date(selectedDate);
    endDate.setHours(toTime.hours, toTime.minutes, 0, 0);
    ...
    if (isToday) {
      ...
      const roundedMinutes = Math.ceil(minutes / timeslot) * timeslot;
      adjustedTime.setMinutes(roundedMinutes, 0, 0);

      adjustedTime.setHours(adjustedTime.getHours() + (doctorProfile?.availabilityAfter || 0));
    ...
    while (currentTime < endDate) {
      timeSlots.push(timeFormatter.format(currentTime));
      currentTime = new Date(currentTime.getTime() + timeslot * 60000);
    }
    ...
    const bookedTimesSet = new Set(appointmentBooked.map(appointment => appointment.time));
    const availableTimeSlots = timeSlots.filter(slot => !bookedTimesSet.has(slot));
